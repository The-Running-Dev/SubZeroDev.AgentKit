...
Describe 'Global front door with isolated homes and local Git origin' {
    ...
    It 'upgrades an exact legacy pointer even when the manifest is schemaVersion 2 and carries no pointers key' {
        # Regression for the bug where a v1->v2 manifest migration drops the 'pointers'
        # key, so the legacy body's own text was never enough on its own to recognize it -
        # every subsequent install treated the untouched legacy block as a collision forever.
        Git-Fixture $f.Origin @('clone','-q',$f.Origin,$f.Root) | Out-Null
        $stateDir = Join-Path $f.Home '.agent-kit-state'
        New-Item -ItemType Directory -Path $stateDir -Force | Out-Null
        $claude = Join-Path $f.Home '.claude/CLAUDE.md'
        ...
        [IO.File]::WriteAllText($claude, $block)
        @{schemaVersion=2;installRoot=$f.Root;source=$f.Origin;registrations=@();pointerBlocks=@{}} |
            ConvertTo-Json -Depth 5 | Set-Content (Join-Path $stateDir 'installed.json')
        $result=Run-Setup $f; Assert-Success $result
        [IO.File]::ReadAllText($claude) | Should -Match 'only when running an AgentKit command'
        (Read-State $f).pointerBlocks.Keys | Should -Contain $claude
    }
