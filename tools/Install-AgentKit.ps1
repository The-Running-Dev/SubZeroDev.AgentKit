...
function Get-Pointer {
    # No @-import here: this pointer lands in the host's global, every-project rules
    # file, and an eager import would load AGENTS.shared.md's full contract into every
...
function Update-Pointer([string] $Path, [switch] $Remove) {
    ...
    if ($matches.Count -eq 1) {
        ...
        # Matching the exact generated legacy body is itself the signature: the install
        # root and marker text baked into it are not something a third party reproduces
        # by accident, and a v1-to-v2 manifest migration carries no 'pointers' key forward
        # to require in addition (issue: legacy pointer could never migrate past schemaVersion 2).
        $legacy = $block.Replace("`r`n", "`n") -ceq $legacyBody
