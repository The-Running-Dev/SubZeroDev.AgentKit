...
**A gate that reached the file but not the rendered lists, or the lists but not the file, is this command failing** — not a discrepancy left for a reader to notice. Count the entries in the artifact's `gates` array and the lines across the three lists; those numbers are equal, and where they are not, nothing is rendered or handed off until they are. It is the cross-check the first bullet below makes against the discovered flags, taken one step later: the flags say what belongs in the file, and this says the prose carries exactly what the file holds.

Three lists, rendered from the validated artifact as read back from disk. All three are required, and the second is the one that matters.
