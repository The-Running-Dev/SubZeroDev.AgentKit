...
Every `tools/*.ps1` path below is relative to the kit install root, not a target repo (`AGENTS.shared.md` § *House conventions* → Home-install convention) — run this command from that root, or resolve it first.
