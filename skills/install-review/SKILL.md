...
- **Do not commit, push, or open a pull request.** This command stops at the report for sign-off. `/install` and `/sync` no longer do (`INSTALL.md` phase 4 step 8) and this one still does, on its own reason rather than theirs: a workflow file that grants CI a repository secret is not something to land before the user has confirmed the App install and the secret exist.
