...
## Claim
`skills/refine/SKILL.md` is a front door for an ask that is not a brief, a contract, or a slice: it routes away first to the owning command where one exists, and otherwise refines the ask into a fixed six-field template. It emits and does not execute, runs at `sonnet`/`medium`, and every constraint in `Binding` must cite a file it was read from.
