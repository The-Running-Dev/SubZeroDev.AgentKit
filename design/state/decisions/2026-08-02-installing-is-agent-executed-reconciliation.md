...
## Claim
`INSTALL.md` is a procedure written for an agent, not a script: classify every artifact as absent, identical, divergent, or occupied, propose a resolution per divergence, stop for sign-off, then apply and log. `skills/install/SKILL.md` is a thin locator that reads it, so `/install` and a direct read of `INSTALL.md` follow the same procedure.
