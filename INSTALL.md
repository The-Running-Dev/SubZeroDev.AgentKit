...
| `tools/*.ps1` | Reporting helpers, currently `Measure-Session.ps1`, `Wait-PullRequestCheck.ps1`, `New-DesignDocs.ps1`, `Sync-Kit.ps1`. Root `tools/` is commonly occupied — classify the directory before writing into it, and stop if it holds something unrelated rather than sharing it |
